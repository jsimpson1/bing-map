...
object TileCombiner extends App {
  ...
  def apply(startPoint: WSG84Coordinate, endPoint: WSG84Coordinate, levelOfDetail: Int) = {
    ...
    val tiles = BingMapUtility.getTilesAndHtmlValues(startPoint, endPoint, levelOfDetail)


    val width = tiles.map( t => t._2.x + t._2.size ).max
    val height = tiles.map( t => t._2.y + t._2.size ).max

    val compositeImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB)

    val gr = compositeImage.createGraphics()
